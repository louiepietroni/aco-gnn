...
def reshape_heuristic(heursitic_vector, pyg_data):
    # print(heursitic_vector.size())
    problem_size = pyg_data.x.shape[0]
    heuristic_matrix = torch.full((2, problem_size, problem_size), 1e-10)

    heuristic_matrix[:, pyg_data.edge_index[0], pyg_data.edge_index[1]] = heursitic_vector.squeeze(dim = -1).t()
    # print(heuristic_matrix.size(), 'mat')
...
# tensor([29.0023, 32.3036, 25.8953, 29.0595])
# tensor([10.2440, 11.2491,  9.3266, 10.2172])