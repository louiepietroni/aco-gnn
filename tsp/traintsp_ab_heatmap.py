...
def validate_best(network, problem_size, n_ants, k_sparse=None):
    ...
    for instance_nodes in dataset:
        pyg_data, distances = get_instance_data(instance_nodes, k_sparse)
        iteration_data = evaluate_iteration_best_vary_ab(network, pyg_data, distances, n_ants, 1, 1)
        validation_data.append(iteration_data)
...
if False:
    ...
    for i, alpha in enumerate(variable_values):
        for j, beta in enumerate(variable_values):
            ...
            data[i][j] = val_data
            print(f'Data for {alpha}, {beta} = {val_data}')
    print(data)
    torch.save(data, f'results/abheatmap_base.pt')
# plt.subplot(1, 3, 1)
