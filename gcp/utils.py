...
def generate_problem_instance(size, p=0.2):
...
def generate_dataset(dataset_type, problem_size, dataset_size):
    ...
    dataset = instances
    Path(f'datasets/gcp/{problem_size}').mkdir(parents=True, exist_ok=True)
    torch.save(dataset, f'datasets/gcp/{problem_size}/{dataset_type}.pt')
    print(f'Generated {dataset_size} instances in: datasets/gcp/{problem_size}/{dataset_type}.pt')

def load_dataset(dataset_type, problem_size):
    dataset_path = f'datasets/gcp/{problem_size}/{dataset_type}.pt'
