...
def generate_problem_instance(size):
    ...
    items = torch.randint(low=1, high=21, size=(size+1, 1), dtype=torch.float)
    items[0] = 0
    return items
...
def generate_dataset(dataset_type, problem_size, dataset_size):
    ...
    Path(f'datasets/scp/{problem_size}').mkdir(parents=True, exist_ok=True)
    torch.save(dataset, f'datasets/scp/{problem_size}/{dataset_type}.pt')
    print(f'Generated {dataset_size} instances in: datasets/scp/{problem_size}/{dataset_type}.pt')

def load_dataset(dataset_type, problem_size):
    dataset_path = f'datasets/scp/{problem_size}/{dataset_type}.pt'
