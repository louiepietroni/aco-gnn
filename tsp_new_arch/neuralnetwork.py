...
class GNNLayer(nn.Module):
    def __init__(self, dimension, activation_function, aggregation_function):
        ...
        self.temp = nn.Linear(dimension*2, dimension)
    ...
    def forward(self, h_in, e_in, edge_index):
        ...
        # print(Vh[edge_index[1]].size())
        # print(Uh[edge_index[0]].size())

        # aggregated_h = self.aggregation_function(sigmoid_e * Vh[edge_index[1]], edge_index[0])
        # temp_thing = (Vh[edge_index[1]] + Uh[edge_index[0]])/2 # This is just mean of self with it's neighbours
        # temp_concat = torch.cat((Vh[edge_index[1]], Uh[edge_index[0]]), dim=1)
        temp_concat = torch.cat((Vh[edge_index[1]], Uh[edge_index[0]]), dim=1)
        # print(temp_concat.size())
        # exit()
        temp_thing = self.temp(temp_concat)

        
        # aggregated_h = self.aggregation_function(sigmoid_e * temp_thing, edge_index[0])
        aggregated_h = self.aggregation_function(sigmoid_e * temp_thing, edge_index[0])
