...
for _ in range(5):
    ...
    visualiseWeights(data.x, sim_heu.pheromones * sim_heu.heuristics, path)
    visualiseWeights(data.x, sim_heu.pheromones * sim_heu.heuristics, sim_heu.two_opt(path)[0])
...
# ax.plot(np.mean(costs_base_local, axis=1), label='Base + Local')
ax.plot(np.mean(costs_heu_local, axis=1), label='Heu + Local')
